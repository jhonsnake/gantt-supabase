...
export default function GanttChart() {
  ...
  const calculateTaskPosition = (task: Task) => {
    const startMonth = task.start_date.getMonth()
    const endMonth = task.end_date.getMonth()
    ...
    const startDay = task.start_date.getDate()
    const endDay = task.end_date.getDate()
    const daysInStartMonth = new Date(task.start_date.getFullYear(), startMonth + 1, 0).getDate()
    const daysInEndMonth = new Date(task.end_date.getFullYear(), endMonth + 1, 0).getDate()
    ...
    const adjustedEndDay =
      visibleEndMonthOffset > 0 ? new Date(task.end_date.getFullYear(), adjustedEndMonth + 1, 0).getDate() : endDay
  ...
  const filteredTasks = tasks.filter((task) => {
    ...
    return (
      ...
      statusNames[task.status as TaskStatus].toLowerCase().includes(searchLower)
    )
  })
                      ...
                      return (
                        ...
                        >
                          <div
                            ...
                            role="button"
                            tabIndex={0}
                            aria-label={`Tarea: ${task.name}, Estado: ${statusNames[task.status as TaskStatus]}, Periodo: ${format(task.start_date, "d MMM", { locale: es })} - ${format(task.end_date, "d MMM", { locale: es })}`}
                            onKeyDown={(e) => {
                          ...
                          >
                            ...
                            {activeTaskId === task.id && <TaskTooltip task={task} />}
                          </div>
                        </div>
                      )
                    })}
